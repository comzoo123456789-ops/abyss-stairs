...
(function (global) {
  ...
  World.prototype.step = function () {
    ...
    for (i = 0; i < this.ents.length; i++) {
      ...
      var mx = e.mx, my = e.my;
      /* 휘두르는 **그 순간에만** 발이 느려진다.
       * ⚠ 전에는 `atkRest > 0` 까지 넣어 두었다. atkRest 는 공격 주기의
       *   남은 시간을 통째로 채우므로(combat.js 의 `Math.max(recover,
       *   cycle - windup)`), 누르고 있으면 **내내 30%** 였다 — 공격하는
       *   동안 계속 기어 다니는 꼴이다.
       * ⚠ 그렇다고 0 으로 두면 휘두르며 그대로 돌진한다. 선딜에만 걸고
       *   값도 0.30 → 0.55 로 올린다. 멈추는 것이 아니라 **무거워지는** 것이다. */
