...
(function (global) {
  ...
  var SKILLS = [
    ...
    { id: "bloodrage", type: "active", name: "광폭화", kind: "buff", icon: "s_cleave",
      ...
      text: "8초간 폭주하여 공격력 +35%, 이동속도 +20% 증가",
      syn: [
        { id: "long", name: "지속 증가", text: "지속 8 → 12초", s: { dur: 4 } },
        { id: "power", name: "한계 파괴", text: "공격력 +35% → +50%", s: { mult: 0.15 } },
        { id: "drain", name: "피의 흡수", text: "폭주 중 타격시 체력 회복", s: { heal: 15 } }
      ] },
    { id: "leap", type: "active", name: "도약 도륙", kind: "nova", icon: "s_stomp",
      cd: 8.0, cast: 0.35, after: 0.20, stam: 25, mult: 2.2, reach: 3.5, push: 1.5,
      text: "적진 한가운데로 도약하여 지면을 가르고 2.2배 피해",
      syn: [
        { id: "heavy", name: "묵직한 착지", text: "위력 2.2 → 3.0배", s: { mult: 0.8 } },
        { id: "wide", name: "광범위 파쇄", text: "반경 3.5 → 4.8칸", s: { reach: 1.3 } },
        { id: "quick", name: "재빠른 도약", text: "재사용 8.0 → 5.5초", s: { cd: -2.5 } }
      ] },
    { id: "shatter", type: "active", name: "대지 파쇄", kind: "swing", icon: "s_stomp",
      cd: 7.0, cast: 0.30, after: 0.20, stam: 22, mult: 2.0, reach: 2.2, arc: 160,
      text: "무게를 실어 대지를 찍어내리며 적의 방어를 쇄쇄한다",
      syn: [
        { id: "brutal", name: "잔혹한 일격", text: "위력 2.0 → 2.8배", s: { mult: 0.8 } },
        { id: "stun", name: "충격 둔화", text: "맞은 적 3초간 50% 둔화", s: { slow: 3 } },
        { id: "guard", name: "파쇄 방벽", text: "시전 시 3초간 방어 +10", s: { armor: 10 } }
      ] },
    { id: "decimate", type: "active", name: "난무 학살", kind: "nova", icon: "s_cleave",
      cd: 10.0, cast: 0.40, after: 0.30, stam: 30, mult: 2.5, reach: 3.0,
      text: "주변의 모든 적을 연속으로 베어넘겨 2.5배 초토화 피해",
      syn: [
        { id: "massacre", name: "대학살", text: "위력 2.5 → 3.5배", s: { mult: 1.0 } },
        { id: "vampire", name: "피의 축제", text: "적중한 적 하나당 체력 회복", s: { heal: 10 } },
        { id: "cd", name: "신속 난무", text: "재사용 10.0 → 7.0초", s: { cd: -3.0 } }
      ] },
    { id: "aegis", type: "active", name: "신성 가호", kind: "buff", icon: "s_guard",
      cd: 14.0, cast: 0.10, after: 0.10, stam: 25, dur: 6.0, armor: 25,
      text: "6초간 신성한 보호막을 형성하여 방어력 +25 대폭 상승",
      syn: [
        { id: "long", name: "은총 연장", text: "지속 6 → 10초", s: { dur: 4 } },
        { id: "solid", name: "철벽 가호", text: "방어력 +25 → +40", s: { armor: 15 } },
        { id: "reflect", name: "빛의 반사", text: "받은 피해의 50% 반사", s: { reflect: 50 } }
      ] },
    { id: "judgment", type: "active", name: "신벌의 빛", kind: "nova", icon: "s_nova",
      cd: 9.0, cast: 0.40, after: 0.25, stam: 28, mult: 2.1, reach: 3.2,
      text: "하늘에서 성스러운 빛을 내리쳐 3.2칸 주변 적 응징",
      syn: [
        { id: "holy", name: "성스러운 징벌", text: "위력 2.1 → 3.0배", s: { mult: 0.9 } },
        { id: "blind", name: "섬광 둔화", text: "맞은 적 4초간 40% 둔화", s: { slow: 4 } },
        { id: "cleanse", name: "정화", text: "사용 시 기력 30 회복", s: { heal: 30 } }
      ] },
    { id: "brand", type: "active", name: "사령의 낙인", kind: "swing", icon: "s_cleave",
      cd: 6.0, cast: 0.25, after: 0.20, stam: 20, mult: 1.8, reach: 2.0, arc: 150,
      text: "검에 암흑의 저주를 담아 베어 낙인을 새긴다",
      syn: [
        { id: "curse", name: "깊은 저주", text: "위력 1.8 → 2.6배", s: { mult: 0.8 } },
        { id: "drain", name: "생명 착취", text: "가한 피해의 20% 흡혈", s: { leech: 20 } },
        { id: "quick", name: "신속 낙인", text: "재사용 6.0 → 4.0초", s: { cd: -2.0 } }
      ] },
    { id: "souldrain", type: "active", name: "영혼 갈퀴", kind: "field", icon: "s_burn",
      cd: 11.0, cast: 0.30, after: 0.25, stam: 32, mult: 0.6, reach: 2.8, dur: 6.0, tick: 0.5,
      text: "바닥에 암흑 지대를 설치하여 영혼을 계속해서 갈퀴질한다",
      syn: [
        { id: "long", name: "심연의 늪", text: "지속 6 → 10초", s: { dur: 4 } },
        { id: "dense", name: "모진 갈퀴", text: "피해 주기 0.5 → 0.3초", s: { tick: -0.2 } },
        { id: "wide", name: "영혼의 광야", text: "반경 2.8 → 3.8칸", s: { reach: 1.0 } }
      ] },
    { id: "stealth", type: "active", name: "은신 암습", kind: "buff", icon: "s_dash",
      cd: 12.0, cast: 0.10, after: 0.10, stam: 20, dur: 5.0,
      text: "그림자 속으로 숨어 이동속도 +30% 및 다음 공격 100% 치명타",
      syn: [
        { id: "long", name: "그림자 동화", text: "지속 5 → 8초", s: { dur: 3 } },
        { id: "lethal", name: "치명 일격", text: "은신 해제 첫 타 피해 2.5배", s: { mult: 1.5 } },
        { id: "fast", name: "전광석화", text: "이동속도 +50% 로 증가", s: { spd: 0.2 } }
      ] },
    { id: "fatalslash", type: "active", name: "급소 베기", kind: "swing", icon: "s_cleave",
      cd: 7.0, cast: 0.20, after: 0.15, stam: 25, mult: 2.8, reach: 1.8, arc: 120,
      text: "적의 치명적인 약점을 정확히 겨냥해 2.8배 대단원 일격",
      syn: [
        { id: "crit", name: "급소 관통", text: "위력 2.8 → 3.8배", s: { mult: 1.0 } },
        { id: "bleed", name: "과다 출혈", text: "맞은 적 5초간 출혈 피해", s: { bleed: 5 } },
        { id: "reset", name: "암살의 맛", text: "적 처치 시 쿨다운 즉시 초기화", s: { reset: 1 } }
      ] },
    { id: "mirrorimage", type: "active", name: "환영 분신", kind: "buff", icon: "s_dash",
      cd: 13.0, cast: 0.20, after: 0.10, stam: 25, dur: 7.0,
      text: "환영을 생성하여 7초간 회피율 +40% 및 공격속도 +30%",
      syn: [
        { id: "long", name: "잔상 유지", text: "지속 7 → 11초", s: { dur: 4 } },
        { id: "dance", name: "환형 춤", text: "공격속도 +30% → +50%", s: { apsPct: 20 } },
        { id: "ambush", name: "기습 분신", text: "사용 즉시 적을 밀쳐냄", s: { push: 2.0 } }
      ] },
    { id: "shadowdance", type: "active", name: "그림자 춤", kind: "dash", icon: "s_dash",
      cd: 8.0, cast: 0.0, after: 0.15, stam: 24, mult: 1.8, dist: 5.5, speed: 25, reach: 1.0,
      text: "그림자처럼 적 사이를 교차하며 쾌속으로 베어넘긴다",
      syn: [
        { id: "far", name: "긴 그림자", text: "이동 거리 5.5 → 8.0칸", s: { dist: 2.5 } },
        { id: "sharp", name: "날카로운 칼날", text: "위력 1.8 → 2.5배", s: { mult: 0.7 } },
        { id: "chase", name: "무한의 춤", text: "적중시 쿨다운 3초 감소", s: { refund: 3 } }
      ] },
    { id: "meteor", type: "active", name: "메테오 스트라이크", kind: "field", icon: "s_burn",
      cd: 14.0, cast: 0.50, after: 0.30, stam: 40, mult: 1.0, reach: 3.5, dur: 5.0, tick: 0.5,
      text: "하늘에서 붉은 운석을 떨어뜨려 5초간 지옥불 장판 생성",
      syn: [
        { id: "apocalypse", name: "종말의 운석", text: "장판 피해 1.0 → 1.8배", s: { mult: 0.8 } },
        { id: "wide", name: "초대형 림", text: "반경 3.5 → 5.0칸", s: { reach: 1.5 } },
        { id: "stun", name: "운석 충격", text: "낙하 시 적을 2초간 기절", s: { stun: 2 } }
      ] },
    { id: "chainlightning", type: "active", name: "연쇄 벼락", kind: "nova", icon: "s_nova",
      cd: 7.0, cast: 0.30, after: 0.20, stam: 26, mult: 2.2, reach: 4.0,
      text: "전기 에너지를 방출하여 넓은 범위의 적들에게 연쇄 감전 피해",
      syn: [
        { id: "highvol", name: "고전압", text: "위력 2.2 → 3.1배", s: { mult: 0.9 } },
        { id: "paralyze", name: "감전 마비", text: "맞은 적 3초간 50% 둔화", s: { slow: 3 } },
        { id: "quick", name: "신속 충전", text: "재사용 7.0 → 4.5초", s: { cd: -2.5 } }
      ] },
    { id: "summonundead", type: "active", name: "해골 군단 소환", kind: "nova", icon: "s_nova",
      cd: 16.0, cast: 0.50, after: 0.30, stam: 35, mult: 1.5, reach: 3.0,
      text: "땅속에서 언령 해골을 부르고 충격파로 피해를 입힌다",
      syn: [
        { id: "army", name: "해골 대군", text: "충격파 위력 1.5 → 2.4배", s: { mult: 0.9 } },
        { id: "curse", name: "저주받은 군단", text: "맞은 적 방어력 -5 저주", s: { armor: -5 } },
        { id: "cd", name: "사령의 부름", text: "재사용 16.0 → 11.0초", s: { cd: -5.0 } }
      ] },
    { id: "decayaura", type: "active", name: "부패의 오라", kind: "field", icon: "s_burn",
      cd: 12.0, cast: 0.30, after: 0.20, stam: 30, mult: 0.5, reach: 3.2, dur: 8.0, tick: 0.6,
      text: "자신 주변에 부패의 아우라를 펼쳐 접촉하는 적을 썩게 만든다",
      syn: [
        { id: "long", name: "영구 부패", text: "지속 8 → 13초", s: { dur: 5 } },
        { id: "toxic", name: "독성 중독", text: "초당 피해 0.5 → 0.9배", s: { mult: 0.4 } },
        { id: "drain", name: "생기 흡수", text: "아우라 피해의 15% 흡혈", s: { leech: 15 } }
      ] }
  ];
